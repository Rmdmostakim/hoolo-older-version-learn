...
function Login() {
  ...
  const comfirmHandler = () =>{
    if(credentials.password.trim().length>5){
      if(isphoneExists){
        ...
        axios.post(`${baseUrl}react/login`,loginCredentials)
        .then((res)=>{
          if (res.status === 202) {
              dispatch(setProfile(res.data));
              dispatch(getCart());
        ...
        axios.post(`${baseUrl}user-registration`,regCredentials)
        .then((res)=>{
          if (res.status === 201) {
              dispatch(setProfile(res.data));
              dispatch(getCart());
    ...
    return (
      <div>
        ...
        <div className='d-flex justify-content-between'>
          <Button variant="warning" size="sm" onClick={changeNumber}><b><AiOutlineLeft/>Change Number</b></Button>
          <Button variant="warning" size="sm" onClick={resnedOtp}><b>Resend OTP<AiOutlineRight/></b></Button>
        </div>
      </div>
    );
  };
  ...
  const usernameView = () =>{
    return (
      <div>
        ...
        <div className="text-center">
          <Button variant="none" size="sm" onClick={nextToUsername} style={{width:'100%',backgroundColor:'#1C405D',color:'white',borderRadius:'2px', margin:'1rem 0rem', padding:'5px'}}>
            Next <AiOutlineRight/>
          </Button>
        </div>
      </div>
  ...
  const resnedOtp = () =>{
    axios.get(`https://shop.hoolo.live/api/verification/${credentials.phone}`);
  }
          ...
          <Col lg={4} md={6} sm={12}>
            <div className='card login' style={{padding:'2rem 1rem'}}>
              <Form>
                ...
                {issetOtp && !issetUsername && usernameView()}
                {issetUsername && passwordView()}
                {isphoneExists && passwordView()}
              </Form>
            </div>
            <div className='card'>
