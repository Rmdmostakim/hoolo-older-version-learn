...
const MyAccount = () => {
	const { id, name, mobile, address, social } = useSelector(
		(state) => state.profile,
	);
	...
	const updateHandler = () => {
		...
		axios
			.post("https://shop.hoolo.live/api/users/update", bodyFormData)
			.then((res) => {
				if (res.status === 200) {
					setShow(false);
	...
	useEffect(() => {
		if (userId) {
			axios.get(`${baseUrl}users/profile/${userId}`).then((res) => {
	...
	const likeBtn = () => {
		axios.get(`${baseUrl}users/likes/${userId}`).then((res) => {
			setLikes(res.data.Likes);
		});
	};
	const followBtn = () => {
		axios.get(`${baseUrl}users/follows/${userId}`).then((res) => {
			setFollows(res.data);
		});
	};

	const orderBtn = () => {
		axios.get(`${baseUrl}orders-by/${userId}`).then((res) => {
			if (res.status === 200) {
				setOrders(res.data);
			}
		});
	...
	const [refundcredentials, setRefundcredentials] = useState({
		user_id: userId,
		order_code: "",
		reason: "",
		account_number: "",
	...
	const refundSubmit = () => {
		if (
			refundcredentials.attachment === null ||
			refundcredentials.reason === "" ||
			refundcredentials.order_code === "" ||
			refundcredentials.account_number === ""
		) {
			...
			axios
				.post("https://shop.hoolo.live/api/create-refunds", bodyFormData)
				.then((res) => {
	...
	const cancelOrder = (order) => {
		...
		axios
			.post("https://shop.hoolo.live/api/cancel-order", data)
			.then((res) => {
				if (res.status === 400) {
			...
			<div>
						...
						<Container fluid className="drop">
							<Row>
								<Col md={12} className="mb-2">
									<Dropdown
										title="Go Live"
										Icon={<img src={golive} alt="golive" />}
										dataHandler={null}
									>
										<h6>
											<Link to="/streaming">Start live streaming</Link>
										</h6>
									</Dropdown>
								</Col>
								<Col md={12} className="mb-2">
											...
											{orders ? (
												<div className="profilefixdiv">
													{orders.map((order, index) => (
														<div
															className="card perCard mt-2 mb-3"
															key={order.id}
														>
															<Row
													...
													{likes.map((data) => (
														<div className="profileitem" key={data.id}>
															<div className="lf_img">
																<Link to={`/videos/${data.video.slug}`}>
																	<img
																		style={{
																			width: "80px",
																			height: "85px",
															...
															<div
																className="l_dateinfo"
																style={{ padding: "5px" }}
															>
																<div className="lf_info">
																	<Link to={`/videos/${data.video.slug}`}>
																		<b>{data.video.title}</b>
																	</Link>
																	<br />
																	By
																	<Link
																		style={{ color: "blue", fontWeight: 400 }}
																		to={`/vendors/videos/${data.video.store.uuid}`}
																	>
																		{" "}
																		{data.video.store.name}
																	</Link>
																</div>
																<div className="lf_date">{data.created_at}</div>
															</div>
														</div>
													))}
												</div>
								...
								<Col md={12} className="mb-2">
									<Dropdown
										...
										Icon={<img src={follow} alt="follow" />}
										dataHandler={followBtn}
									>
										<div>
											{follows ? (
												<div className="profilefixdiv">
													{follows.map((data) => (
														<div className="profileitem" key={data.id}>
															...
															<div
																className="f_dateinfo"
																style={{ padding: "5px" }}
															>
																<div className="lf_info">
																	<Link
																		to={`/vendors/videos/${data.store_uuid}`}
																	>
																		<b>{data.store_name}</b>
																	</Link>
																</div>
								...
								<Col md={12} className="mb-2">
									<Dropdown
										...
										Icon={<img src={details} alt="details" />}
										dataHandler={() => null}
									>
										<div>
											<p>
											...
											<p>
												...
												{userInfo && userInfo.mobile && userInfo.mobile}
											</p>
											<p>
										...
										<div className="editprofile">
											<div>
												<iconify-icon icon="ep:edit" width="19"></iconify-icon>
											</div>
											<div>
												<Button
													style={{ paddingInline: "0px" }}
				...
				<Modal show={show} onHide={handleClose}>
					...
					<Modal.Footer>
						<Button
							variant="none"
							style={{ backgroundColor: "#ffce2c", borderRadius: "5px" }}
							onClick={handleClose}
						>
							Cancel
						</Button>
				...
				<Modal show={showRefund} onHide={handleCloseRefund}>
					<Modal.Header closeButton>
						<Modal.Title>
							<small>Request Refund</small>
						</Modal.Title>
					</Modal.Header>
					<Modal.Body>
						<Form>
							{refundErrors && refundErrors.hasOwnProperty("exists") && (
								<Alert variant="danger">{refundErrors.exists}</Alert>
							)}
							<Form.Group>
								<Form.Label className="text-black font-Normal">
									Order Code
								</Form.Label>
								{refundErrors && refundErrors.hasOwnProperty("order_code") && (
									<Alert variant="danger">{refundErrors.order_code}</Alert>
								)}
								<Form.Control
									type="text"
							...
							<Form.Group>
								...
								{refundErrors && refundErrors.hasOwnProperty("reason") && (
									<Alert variant="danger">{refundErrors.reason}</Alert>
								)}
								<Form.Control
									type="textarea"
							...
							<Form.Group>
								...
								{refundErrors &&
									refundErrors.hasOwnProperty("account_number") && (
							...
							<Form.Group>
								<Form.Label>
									Pictures or videos to validate refund request
								</Form.Label>
								{refundErrors && refundErrors.hasOwnProperty("attachment") && (
									<Alert variant="danger">{refundErrors.attachment}</Alert>
								)}
								<input
									multiple
									name="attachment"
